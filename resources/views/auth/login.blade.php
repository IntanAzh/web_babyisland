<x-layout>
    ...
    <div class="container mx-auto flex items-center justify-center h-[100vh]">
        ...
        <div class="bg-[#FEF2E0] hadow-md rounded-2xl px-10 py-12 w-[400px]">
            ...
            <form class="space-y-6" id="register" method="POST" action="/login">
                @csrf
                <div>
                    <label for="username" class="block text-sm text-gray-600 mb-1">Username
                        @error('username')
                ...
                <div>
                    <label for="password" class="block text-sm text-gray-600 mb-1">Password
                        @error('password')
                            <p class="text-red-600 text-sm mb-1">{{ $message }}</p>
                        @enderror
                    </label>
